...
@interface CDVInAppBrowser () {
    ...
    /**
     * Elli Rego added line below to facilitate opening external links in system browser.
     */
    NSString* _internalBaseUrl;
    /**
...
- (BOOL)webView:(UIWebView*)theWebView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType
{
     ...
     */
    // Check if we're navigating to an external link
    if (isTopLevelNavigation && [self isRealURL:url] && [self isExternalUrl:url]) {
        [self performSelector:@selector(openExternalLink:) withObject:url];
        return NO;
    /**
     * End of Elli Rego's additions.
     */
...
- (BOOL)isRealURL:(NSURL*)url
{
    NSString* urlString = [url absoluteString];
    
    if (!([[url scheme] isEqualToString:@"gap-iab"]) && ([urlString rangeOfString:@"http"].location == 0)) {
        return YES;
    }
...
- (BOOL)isExternalUrl:(NSURL*)url
{
    ...
    if ([urlString rangeOfString:_internalBaseUrl].location == NSNotFound) {
        return YES;
    }
...
- (void)setInternalBaseUrl:(NSURL*)url
{
    NSString* internalBaseUrl;
    NSString* urlHost = url.host;
    NSString* www = @"www.";
    
    if ([urlHost rangeOfString:www].location == 0) {
        internalBaseUrl = [urlHost substringFromIndex:4];
    } else {
        internalBaseUrl = urlHost;
    }
